...
begin
 ...
 // end else if ((rightstr(inttostr(timeMM),2) > '15') and (rightstr(inttostr(timeMM),2) < '25')) OR ((rightstr(inttostr(timeMM),2) > '45') and (rightstr(intTostr(timeMM),2) < '55'))   then begin
  end else if (timeMM > 900) and (timeMM <= 1330) then begin
       company_gubun :=' where sosok < 2 and curPrice > 1000 and SaleAmount > 0'; //and  SalesProfit > 10 and DEBTTOTAL <= SALEAMOUNT * 0.6';
       button2click(nil);
       Button7Click(nil);
       Button8Click(nil);
       form1.Caption :=  formatdatetime('YYYY-MM-DD hh:mm',now);

  end else if (timeMM > 1330) and (timeMM < 1535) then begin
       button2click(nil);
       Button3Click(nil);
  end;
      ...
      try
         try
          ...
          if not checkbox2.Checked  then  Sql := 'select * from Main_Comp ' + company_gubun + ' order by ccode'
              ...
              Sql := 'SELECT CCODE, DDATE FROM (SELECT CCODE, DDATE, (HIGHPRICE - PREPRICE1) * 100/NULLIF(PREPRICE1,0) AS MXRATE FROM DAILY_KOSP WHERE DDATE =''' + CallDay + ''') A WHERE A.MXRATE > 10 ';
...
begin
        ...
        ADOQuery3.Close;
        ADOQuery3.SQL.Clear;
        sql := 'SELECT A.CCODE, A.DDATE, B.COMPANY, A.TIMEB, A.ENDPRICE, A.BUYPRICE,A.NOUSE  FROM  MIN_TD_TEMP A INNER JOIN MAIN_COMP B ON(A.CCODE = B.CCODE)  WHERE NOUSE =0  ORDER BY TIMEA';
        ADOQuery3.SQL.Add(sql);
        ADOQuery3.Open;
        while NOT ADOQuery3.Eof  do begin
